...
class FlintSparkSkippingIndexSqlITSuite extends FlintSparkSuite {
  ...
  test("analyze skipping index on invalid query") {
    the[IllegalArgumentException] thrownBy {
      sql(s"ANALYZE SKIPPING INDEX WITH SELECT age FROM $testTable")
    }
  }

  test("analyze skipping index with sql query") {
    val result =
      sql(s"ANALYZE SKIPPING INDEX WITH SELECT age FROM $testTable WHERE age < 5")
  ...
  test("analyze skipping index with sql query with IN function") {
    val result =
      sql(s"ANALYZE SKIPPING INDEX WITH SELECT age FROM $testTable WHERE age IN(2, 4)")
